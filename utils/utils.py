        ...
        try:
            ...
            if content_type == ContentType.TEXT:
                await bot.send_message(
                    chat_id=chat_id, text=text, reply_markup=main_contact_kb(chat_id))
            elif content_type == ContentType.PHOTO:
                await bot.send_photo(
                    chat_id=chat_id, photo=photo_id, caption=caption, reply_markup=main_contact_kb(chat_id))
            elif content_type == ContentType.DOCUMENT:
                await bot.send_document(
                    chat_id=chat_id, document=document_id, caption=caption, reply_markup=main_contact_kb(chat_id))
            elif content_type == ContentType.VIDEO:
                await bot.send_video(
                    chat_id=chat_id, video=video_id, caption=caption, reply_markup=main_contact_kb(chat_id))
            elif content_type == ContentType.AUDIO:
                await bot.send_audio(
                    chat_id=chat_id, audio=audio_id, caption=caption, reply_markup=main_contact_kb(chat_id))
            good_send += 1
        except Exception as e:
            print(e)
            bad_send += 1
